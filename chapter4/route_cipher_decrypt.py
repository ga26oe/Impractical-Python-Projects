...
def validate_col_row(cipherlist):
    """Check that the input columns & rows are valid vs. message length"""
    factors = []
    len_cipher = len(cipherlist)
    for i in range(2, len_cipher):
        if len_cipher % i == 0:
            factors.append(i)
    print("Length of cipher = {}".format(len_cipher))
...
def key_to_int(key):
    ...
    if len(key_int) != COLS or key_int_lo < -COLS or key_int_hi > COLS \
        or 0 in key_int:
        print("\nError - Problem with key. Termination.", file=sys.stderr)
        sys.exit(1)
    else:
        return key_int

def build_matrix(key_int, cipherlist):
    """Turn every n times in a list into a new item in a list of lists."""
    translation_matrix = [None] * COLS
    start = 0
    stop = ROWS
    for k in key_int:
        if k < 0: #read from bottom up
            col_items = cipherlist[start:stop]
        elif k > 0: #read from top to bottom
...
def decrypt(translation_matrix):
    """Loop throughnested lists popping last item to string"""
    plaintext = ""
    for i in range(ROWS):
