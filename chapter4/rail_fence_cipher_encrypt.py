...
def build_rails(message):
    ...
    evens = message[::2] #start at 0 step by 2
    odds = message[1::2] #start at 1 step by 2
    rails = evens + odds
    return rails

def encrypt(rails):
    """Split letters in ciphertext into chunks of 5 and join to make string."""
    ciphertext = ' '.join([rails[i:i+5] for i in range(0, len(rails), 5)]) # split ciphertext into chunks of 5 
